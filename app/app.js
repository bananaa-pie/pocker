...
function defaultState() {
  return {
    ...
    levels: [
      ...
      { sb: 100,  bb: 200,  ante: 0,    dur: 12, colorUp: true, cuNote: 'Убрать со стола все 10 и 20' }, // 6
      ...
      { sb: 200,  bb: 400,  ante: 400,  dur: 12, colorUp: true, cuNote: 'Убрать со стола все 50' }, // 8
...
function advance(delta, autorun) {
  ...
  // autorun keeps the clock going (even across the rebuy-close — just shows the note);
  // a manual crossing of the rebuy boundary still pauses so the director can announce.
  const run = autorun ? true : (closing ? false : wasRunning);
...
// ——— money helpers ———
// Точный расчёт: округляем до рубля (не до 100 ₽), остаток от округления — 1-му месту,
// чтобы сумма выплат в точности равнялась призовому фонду.
function payouts(bank, pcts, step = 1) {
...
function showToast(text, undoFn, ms) {
  ...
  el.innerHTML = `<span>${esc(text)}</span>`
    + (undoFn ? `<button class="btn btn-ghost" id="toast-undo">Отменить</button>` : `<button class="btn btn-ghost" id="toast-x">✕</button>`);
  el.classList.add('show');
  ...
  else { const x = document.getElementById('toast-x'); if (x) x.onclick = () => el.classList.remove('show'); }
}
...
function onLevelField(e) {
  ...
  const val = f === 'cuNote' ? v : (v === '' ? 0 : Math.max(0, +v || 0)); // cuNote is text, the rest numeric
  setState(s => ({ levels: s.levels.map((l, k) => k === i ? { ...l, [f]: val } : l) }));
}
function onCfg(e) {
...
function derive() {
  ...
  const prizeAmts = payouts(bank, splits);
  ...
  const rows = s.levels.map((lv, i) => {
    ...
    <tr>
      ...
      <td style="text-align:center;min-width:150px">
        ...
        ${lv.colorUp ? `<input class="input" style="margin-top:4px;font-size:11px;width:100%" placeholder="что убрать, напр. 10 и 20" value="${esc(lv.cuNote || '')}" data-idx="${i}" data-field="cuNote">` : ''}
      </td>
      <td><button class="btn btn-icon pk-btn-danger" title="Удалить" data-idx="${i}" data-action="removeLevel">×</button></td>
    </tr>`;
  }).join('');
...
function runningHtml(d) {
  ...
  const tile = (label, value, big) => `
    <div class="pk-tile${big ? ' pk-tile-lg' : ''}">
      <div class="pk-tile-cap">${label}</div>
      <div class="pk-num pk-tile-val" style="font-family:var(--font-heading)">${value}</div>
    </div>`;
  const infoTiles = `
    <div class="pk-info">
      ${d.curIsBreak ? tile('Статус', '⏸ Перерыв', true) : tile('Блайнды', fmt(cur.sb) + ' / ' + fmt(cur.bb), true)}
  ...
  const bannerInner = `
    ${d.bubble ? `<div class="pk-bubble"><span class="dot"></span><span>Бабл — остался один вылет до призовой зоны</span></div>` : ''}
    ${cur.colorUp ? `<div class="pk-colorup"><span class="dot"></span><span>Замена фишек: ${esc(cur.cuNote || 'убрать младшие фишки')}</span></div>` : ''}`;
  const banners = (d.bubble || cur.colorUp) ? `<div class="pk-banners">${bannerInner}</div>` : '';
  ...
  <main class="pk-run">
   ...
   <div class="pk-run-top">
    ...
    <div class="pk-below-grid">
      ...
      <div class="pk-scroll pk-run-structure">
...
function modalsHtml(d) {
  ...
  if (s.levelEnd) {
    ...
    const ended = s.levels[s.currentLevel] || {};
    const isColorUp = !!ended.colorUp && !d.lastLevel;
    const btnText = d.lastLevel ? 'К расчёту призовых' : (nextIsBreak ? '► Начать перерыв' : '► Начать следующий уровень');
    if (isColorUp) {
      // пауза на замену фишек — с описанием, что убрать
      html += `
      <div class="dialog-backdrop">
        <div class="dialog pk-glow" style="text-align:center;align-items:center;border-color:var(--color-accent)">
          <span style="font-size:11px;letter-spacing:.16em;text-transform:uppercase;color:var(--color-accent)">Замена фишек · color up</span>
          <div class="dialog-title" style="font-size:30px">Проведите замену</div>
          <div class="pk-num" style="font-family:var(--font-heading);font-size:34px;line-height:1.1;color:var(--color-accent);margin:2px 0">${esc(ended.cuNote || 'Убрать младшие фишки со стола')}</div>
          <p class="text-muted" style="font-size:12px">Таймер на паузе, пока не проведёте замену. Затем продолжите.</p>
      ...
      const title = d.curIsBreak ? 'Перерыв окончен' : ('Уровень ' + d.blindNo + ' окончен');
      html += `
      <div class="dialog-backdrop">
        <div class="dialog pk-glow" style="text-align:center;align-items:center;border-color:var(--color-accent)">
